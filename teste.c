...
int main(){
	...
	if( memcmp (&un_stuf, &packet, (tamanho_deframed-tamanho_destuffed)) != 0)	
		...
		/*UNPACKING*/
	if(unpack_data(un_pak, 1, un_stuf) == -1)
		printf("Falhou no unpack_data() \n");

	if( memcmp (&un_pak, &dados, (tamanho_deframed-tamanho_destuffed - 4)) != 0)	
		{printf("Falhou no unpack_data(), memcmp \n");return -1;}
