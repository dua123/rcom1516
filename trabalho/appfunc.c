...
int Logic_Emissor()
{
    ...
    Appdata.filesize = total_file_size;
	...
	if(Appdata.filesize < DATAMAXSIZE)
		temp_size = get_chunk(next_chunk, Appdata.filesize, progresso_do_envio*DATAMAXSIZE);	
	else
	{
        	temp_size = get_chunk(next_chunk, DATAMAXSIZE, progresso_do_envio*DATAMAXSIZE);
		Appdata.filesize -= DATAMAXSIZE;	
	}        
	temp_size = packup_data(Appdata.pack_sent, progresso_do_envio, next_chunk, temp_size);
...
int Logic_Recetor()
{
	...
	unpack_data(dados_obtidos, progresso_do_envio, Appdata.pack_received);
    ...
    //ESPERA PELO COMANDO FINAL
        ...
        size_read = llread(Appdata.fd_porta, Appdata.pack_received);
        if ( size_read == -1)
        {
		printf("llread(): ERRO \n");    
		return -1;    
